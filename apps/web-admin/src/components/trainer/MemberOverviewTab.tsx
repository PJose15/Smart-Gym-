...
export function MemberOverviewTab({ memberId }: { memberId: string }) {
  ...
  if (!data) return <p style={{ color: '#EF4444' }}>Failed to load member details.</p>;
  ...
  return (
    <div>
      ...
      <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{
          width: 56, height: 56, borderRadius: '50%', backgroundColor: '#334155',
        ...
        <div>
          ...
          <div style={{ color: '#94A3B8', fontSize: 13 }}>{data.email}</div>
      ...
      <div style={statGrid}>
        <div style={statCard}>
          <div style={{ color: '#64748B', fontSize: 11, marginBottom: 4 }}>Total Sessions</div>
          <div style={{ fontSize: 20, fontWeight: 700 }}>{data.total_sessions}</div>
        </div>
        <div style={statCard}>
          <div style={{ color: '#64748B', fontSize: 11, marginBottom: 4 }}>Current Streak</div>
          <div style={{ fontSize: 20, fontWeight: 700 }}>{data.current_streak}d</div>
        </div>
        <div style={statCard}>
          <div style={{ color: '#64748B', fontSize: 11, marginBottom: 4 }}>Score</div>
        ...
        {data.body_metrics?.weight_lbs && (
          <div style={statCard}>
            <div style={{ color: '#64748B', fontSize: 11, marginBottom: 4 }}>Body Weight</div>
            <div style={{ fontSize: 20, fontWeight: 700 }}>{data.body_metrics.weight_lbs} lbs</div>
          </div>
        )}
        <div style={statCard}>
          <div style={{ color: '#64748B', fontSize: 11, marginBottom: 4 }}>Last Session</div>
      ...
      {data.top_machines.length > 0 && (
        <div style={{ ...cardStyle, marginTop: 16 }}>
          <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 600, color: '#94A3B8' }}>Top Machines</h3>
          {data.top_machines.map((m, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: i < data.top_machines.length - 1 ? '1px solid #0F172A' : 'none', fontSize: 13 }}>
              <span>{m.machine_name}</span>
              <span style={{ color: '#94A3B8' }}>{m.sessions} sessions</span>
            </div>
          ))}
        </div>
      )}
