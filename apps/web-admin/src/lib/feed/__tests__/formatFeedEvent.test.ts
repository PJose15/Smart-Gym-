...
describe('formatFeedEvent', () => {
  describe('pr_weight', () => {
    it('returns lbs-formatted string when unit=lbs', () => {
      const event = baseEvent({
        event_type: 'pr_weight',
        context_data: { best_weight_lbs: 225 },
      });
      expect(formatFeedEvent(event, 'lbs')).toBe(
        'Alice hit a new personal best — 225 lbs!'
      );
    });
    ...
    it('converts to kg when unit=kg', () => {
      ...
      expect(formatFeedEvent(event, 'kg')).toBe(
        'Alice hit a new personal best — 102 kg!'
      );
    });
    ...
    it('uses "Member" when member_name is empty', () => {
      const event = baseEvent({
        event_type: 'pr_weight',
        member_name: '',
        context_data: { best_weight_lbs: 225 },
      ...
      expect(formatFeedEvent(event, 'lbs')).toBe(
        'Member hit a new personal best — 225 lbs!'
      );
    });
  });
  ...
  describe('goal_reached', () => {
    it('includes machine_name and converts target to kg', () => {
      ...
      expect(formatFeedEvent(event, 'kg')).toBe(
        'Alice hit their goal on Leg Press — 143 kg!'
      );
    });
    ...
    it('omits machine_name when missing', () => {
      ...
      expect(formatFeedEvent(event, 'lbs')).toBe('Alice hit their goal — 100 lbs!');
    });
  ...
  describe('workout_share', () => {
    it('formats completed share with volume + PRs in kg', () => {
      ...
      expect(formatFeedEvent(event, 'kg')).toBe(
        'Alice finished training — 5.7k kg, 2 PRs'
      );
    });
    ...
    it('pluralizes PR correctly (singular)', () => {
      ...
      expect(formatFeedEvent(event, 'lbs')).toBe(
        'Alice finished training — 500 lbs, 1 PR'
      );
    });
    ...
    it('returns fallback for non-completed shares', () => {
      const event = baseEvent({
        event_type: 'workout_share',
        description: 'Alice is training today',
        context_data: { share_status: 'training', volume_lbs: 0, prs_hit: 0 },
      });
      expect(formatFeedEvent(event, 'kg')).toBe('Alice is training today');
    });
  ...
  describe('passthrough event types', () => {
    it('streak_milestone → returns description unchanged', () => {
      const event = baseEvent({
        ...
        description: 'Alice is on a 7-day streak!',
        context_data: { streak: 7 },
      });
      expect(formatFeedEvent(event, 'kg')).toBe('Alice is on a 7-day streak!');
    });

    it('level_up → returns description unchanged', () => {
      const event = baseEvent({
        event_type: 'level_up',
        description: 'Alice reached Level 5!',
        context_data: { new_level: 5 },
      });
      expect(formatFeedEvent(event, 'kg')).toBe('Alice reached Level 5!');
    });

    it('achievement_earned → returns description unchanged', () => {
      const event = baseEvent({
        event_type: 'achievement_earned',
        description: 'Alice earned "First Steps"',
        context_data: { achievement_code: 'first_steps' },
      });
      expect(formatFeedEvent(event, 'kg')).toBe('Alice earned "First Steps"');
    });
